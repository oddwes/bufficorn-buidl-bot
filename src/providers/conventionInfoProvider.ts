...
async function getMenuUrls() {
  try {
    ...
    urls.add("https://www.ethdenver.com/tickets");
  ...
  try {
    ...
    const pageData = {
      ...
      description: decodeHtml(
        content
          .match(
            /<section class="openhours-activations">.*?<p.*?>(.*?)<\/p>/s
          )?.[1]
          ?.trim() || ""
      ),
    ...
    const openSection = root.querySelector(
      'section.section_activity_time:has(div:contains("Open:"))'
    );
    const closeSection = root.querySelector(
      'section.section_activity_time:has(div:contains("Closes:"))'
    );

    if (openSection) {
      const dateDivs = openSection.querySelectorAll("div.activation_date");
      pageData.schedule.opens = decodeHtml(
        dateDivs[0]?.textContent?.trim() ||
          dateDivs[1]?.textContent?.trim() ||
          ""
      );
    }
    ...
    if (closeSection) {
      ...
      pageData.schedule.closes = decodeHtml(
        dateDivs[0]?.textContent?.trim() ||
          dateDivs[1]?.textContent?.trim() ||
          ""
      );
    ...
    const faqMatches = (content.match(
      /<div class="text-size-medium-3 faq-header">(.*?)<\/div>/g
    ) || []) as string[];

    // Updated answer parsing to handle both formats
    const faqSections = root.querySelectorAll(
      ".home_12_frequently-asked-questions_answer"
    );
    ...
    pageData.faqs = faqMatches.map((q, i) => ({
      question: decodeHtml(
        q
          .replace(/<div class="text-size-medium-3 faq-header">/, "")
          .replace(/<\/div>/, "")
          .trim()
      ),
      answer: faqAnswers[i] || "",
...
async function getConventionInfo() {
  ...
  const allContent = results.filter((content) => content).join("\n");
